...
- The token is cached in `_suiteToken` and reused across tests.
- `[SetUp]` calls `EnsureSuiteTokenAsync()` before every test.

### 2) Refresh strategy (suite-level only)
- If `_suiteToken` is valid, it is reused.
- If `_suiteToken` is expired, a single thread refreshes it under `_suiteLock`.
...
### B) Add role-aware positive helper now (future-ready)
Without changing runtime behavior today, add an extension point:
- `UsePositiveAuthOrInconclusive(flowName, role = "default")`

Current implementation can map `default -> validCredentials`.
Future role-based login can map role-specific credentials without changing test body patterns.
...
## Role-based Login Readiness
The current architecture is compatible with future role-based auth:
- Token lifecycle (cache + refresh) can remain suite-scoped.
- Role dimension can be added as an additional key around token cache (for example, per-role suite token entries).
- Tests can stay stable if they call role-aware intent helpers rather than raw auth primitives.
